...
const { authenticate, enforceToolAccess } = require('./_auth-node');
...
module.exports = async (req, res) => {
  ...
  if (auth.status !== 200) return res.status(auth.status).json({ success: false, error: auth.error });
  const blocked = await enforceToolAccess(auth.user, SLUG);
