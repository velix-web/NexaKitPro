...
export default async function handler(req) {
  ...
  // attacker can still spoof an Origin header from outside a browser), but
  // combined with the rate limit below it stops the common cases: some
  // other site's JS hitting this endpoint through a visitor's browser, and
  // a bare non-browser request (curl/Postman/etc) that carries no Origin
  // at all — a real browser POST always sends one, so requiring it here
  // rejects non-browser callers instead of silently letting them through.
  const origin = req.headers.get('origin');
  const host = req.headers.get('host');
  if (!origin || !host) return json({ success: false }, 403);
  try {
